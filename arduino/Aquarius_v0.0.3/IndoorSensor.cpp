...
bool IndoorSensor::begin(TwoWire& wire, uint8_t sdaPin, uint8_t sclPin) {
  ...
  bool baroFound = beginBarometric();
#if AQUARIUS_ENABLE_DHT
  dht_.begin();
  delay(2100);
  readDht();
...
#endif
  combineReadings();
  return reading_.temperatureValid || reading_.humidityValid || reading_.pressureValid;
}
...
bool IndoorSensor::beginBarometricAt(uint8_t address) {
  ...
  uint8_t id = readRegister(address, CHIP_ID_REGISTER);
  Serial.printf("I2C device 0x%02X reports chip ID 0x%02X\n", address, id);
  ...
  if ((id == 0x56 || id == 0x57 || id == 0x58) && bmp_.begin(address)) {
...
uint8_t IndoorSensor::readRegister(uint8_t address, uint8_t reg) {
  ...
  if (wire_->requestFrom((int)address, 1, true) != 1) return 0xFF;
...
void IndoorSensor::readDht() {
  ...
  dhtResponding_ = !isnan(dhtTemperatureC_) && !isnan(dhtHumidityPct_);
#else
