...
v0.1.2 uses the CYD’s XPT2046 **touchscreen** to feed the fish at the tapped location.
...
Touch-screen feeding is implemented. A tap places food near the tapped X coordinate and the fish react to that position.
...
### v0.1.2 — Behaviour pass

- dashboard layout frozen;
- four common-fish personalities;
- independent cruising targets and voluntary turns;
- eased acceleration;
- approach, frenzy and recovery feeding states;
- species-specific squid, octopus and jellyfish motion.
...
## Weather Ecology — implemented in v0.1.3

Weather data is treated as an input to the aquarium ecosystem rather than a decorative label. The engine maintains smoothed ecology values for daylight, cloud, fog, rain, snow, storm intensity, wind, fish activity, preferred depth, surface energy and bubble rate.

The key design rule is **transition, not mode switching**. When the Open-Meteo weather code changes, aquarium behaviour and colour move toward the new state over time. This prevents sudden jumps from a calm sunny tank to a dark storm tank.

Sunrise and sunset are also ecological inputs. Aquarius uses the fetched local sunrise/sunset times to create a twilight window around each event, so the aquarium gradually wakes and settles rather than simply changing at a fixed clock hour.

Current relationships:

- **Clear:** brighter water, higher fish activity and more bubbles.
- **Cloud/overcast:** progressively muted light and slightly calmer behaviour.
- **Rain/showers:** rougher surface, splashes/rain particles, reduced activity and deeper swimming.
- **Thunderstorm:** strongest surface movement, deeper sheltering behaviour and short lightning events.
- **Snow/snow showers:** drifting particles, low activity and deeper swimming.
- **Fog:** subtle haze, reduced contrast and calmer fish.
- **Wind:** increases surface motion and plant sway.
- **Night:** low activity and restrained bioluminescent specks; nocturnal special events remain possible.

Future weather work should extend *behaviour* before adding more visual clutter: shelter seeking, species-specific weather responses and interaction with decorations are preferred over simply adding more particles.
