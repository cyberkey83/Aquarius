/*
 * Aquarius v0.1.12 — Showcase & Release Testing
 *
 * Focus:
...
constexpr uint8_t DEMO_SCENE_COUNT = 33;
...
void exitDemo(unsigned long nowMs) {
  demoOpen = false; displayManager.clearDemo(nowMs); settingsOpen = true; settingsPage = 3; drawSettings();
}
...
void drawDiagnostics() {
  ...
  canvas.drawString("AQUARIUS DIAGNOSTICS", 10, 8, 2);
...
void drawSettings() {
  ...
  canvas.setTextColor(TFT_DARKGREY, TFT_BLACK);
  canvas.setTextDatum(TR_DATUM);
  const char* titles[] = {"AQUARIUM", "SYSTEM", "WI-FI", "ABOUT"};
  canvas.drawString(titles[settingsPage % 4], 310, 8, 2);
  canvas.drawFastHLine(8, 29, 304, TFT_DARKGREY);
  ...
  if (settingsPage == 0) {
    ...
    compactInfoRow(39,  "Firmware", "v0.1.12");
...
void pollTouch(unsigned long nowMs) {
  ...
  if (!touched && touchWasDown && touchDownMs != 0) {
    ...
    if (demoOpen) {
      ...
      if (settingsPage == 3 && touchDownY >= 35 && touchDownY < 69 && held >= 4000UL) {
        enterDemo(nowMs);
      } else if (settingsPage == 3 && touchDownY < 35 && held >= 4000UL) {
...
void setup() {
  ...
  Serial.println();
  Serial.println("========================================");
  Serial.println(" AQUARIUS v0.1.12");
