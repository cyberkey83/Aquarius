...
bool WeatherService::fetch() {
  ...
  if (!http.begin(url)) {
    Serial.println(
        "Open-Meteo: HTTP client failed to start.");
  ...
  if (parsed) {
    ...
    Serial.println(
        "Open-Meteo response could not be parsed.");
  }
...
bool WeatherService::parseResponse(const String& payload) {
  ...
  const char* sunrise =
      document["daily"]["sunrise"][0] | "";

  const char* sunset =
      document["daily"]["sunset"][0] | "";

  if (strlen(sunrise) >= 16) {
    strlcpy(
        reading_.sunriseText,
        sunrise + 11,
        sizeof(reading_.sunriseText));
  }
