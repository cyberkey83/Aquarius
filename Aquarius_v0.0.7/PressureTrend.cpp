...
void PressureTrend::addSample(float pressureHpa) {
  ...
  nextIndex_ =
      (nextIndex_ + 1) % AQUARIUS_PRESSURE_HISTORY_SIZE;

  if (count_ < AQUARIUS_PRESSURE_HISTORY_SIZE) {
    count_++;
  }
...
float PressureTrend::changeHpa() const {
  ...
  const uint8_t oldestIndex =
      count_ < AQUARIUS_PRESSURE_HISTORY_SIZE
          ? 0
          : nextIndex_;
...
PressureTrendDirection PressureTrend::direction() const {
  if (count_ < 3) {
    return PressureTrendDirection::Learning;
  }
