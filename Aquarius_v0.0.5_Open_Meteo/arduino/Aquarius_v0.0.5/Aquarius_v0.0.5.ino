...
void drawHeader() {
  ...
  tft.setTextDatum(TL_DATUM);
  tft.setTextColor(TFT_DARKGREY, TFT_BLACK);
  tft.drawString("AQUARIUS v0.0.5", 8, 6, 2);
...
void drawFooter() {
  ...
  // Pressure remains an indoor/barometric measurement.
  tft.setTextDatum(TL_DATUM);
  tft.setTextColor(TFT_DARKGREY, TFT_BLACK);
  tft.drawString("PRESS", 10, 207, 1);
  ...
  tft.setTextDatum(TR_DATUM);
  tft.setTextColor(
      outdoor.valid && !weatherService.isStale()
          ? TFT_GREEN
          : TFT_YELLOW,
      TFT_BLACK);
  tft.drawString(weatherService.statusText(), 310, 204, 1);

  tft.setTextColor(TFT_DARKGREY, TFT_BLACK);
  tft.drawString(AQUARIUS_LOCATION_NAME, 310, 219, 1);
...
void setup() {
  ...
  Serial.println();
  Serial.println("========================================");
  Serial.println(" AQUARIUS v0.0.5");
