...
void DisplayManager::configureAquarium(uint8_t fishCount, uint8_t bubbleLevel, uint8_t plantLevel,
  ...
  aquarium_.configure(fishCount, bubbleLevel, plantLevel, plantLength, animationLevel,
                      weatherEffects, eventFrequency, jellyfishEnabled);
}
...
void DisplayManager::drawBootScreen(const String& status, unsigned long nowMs) {
  ...
  canvas_.setTextColor(AquariusTheme::INDOOR);
  canvas_.drawString("ATMOQUARIUM", AquariusTheme::CENTRE_X, 38, 4);

  canvas_.setTextColor(AquariusTheme::PRIMARY);
  canvas_.drawString("Desktop Companion", AquariusTheme::CENTRE_X, 66, 2);

  canvas_.setTextColor(AquariusTheme::MUTED);
  canvas_.drawString("v0.1.13", AquariusTheme::CENTRE_X, 87, 2);
  ...
  // Boot bubbles and original stone-head aquarium ornament.
  for (int i = 0; i < 8; ++i) {
    const unsigned long period = 1800UL + i * 190UL;
    const int bx = 28 + ((i * 43) % 265);
    const int by = 181 - static_cast<int>(((nowMs + i * 337UL) % period) * 76UL / period);
    const int br = 1 + (i % 3 == 0);
    canvas_.drawCircle(bx, by, br, AquariusTheme::OUTDOOR);
  }
  const uint16_t stone = canvas_.color565(55, 66, 70);
  const uint16_t stoneDark = canvas_.color565(27, 35, 38);
  canvas_.fillRoundRect(270, 118, 20, 30, 4, stone);
  canvas_.fillRect(266, 145, 29, 6, stoneDark);
  canvas_.fillRect(274, 125, 12, 4, stoneDark);
  canvas_.fillRect(276, 134, 10, 3, stoneDark);
  canvas_.drawPixel(275, 127, TFT_BLACK); canvas_.drawPixel(284, 127, TFT_BLACK);

  canvas_.setTextColor(AquariusTheme::SECONDARY);
  ...
  canvas_.setTextColor(AquariusTheme::ACCENT);
  canvas_.drawString("cyberkey83", AquariusTheme::CENTRE_X, 202, 2);
  canvas_.setTextColor(AquariusTheme::MUTED);
  canvas_.drawString("github.com/cyberkey83/AtmoQuarium", AquariusTheme::CENTRE_X, 224, 1);
}
...
void DisplayManager::drawHeader(
  ...
  canvas_.setTextColor(AquariusTheme::PRIMARY);
  canvas_.drawString("ATMOQUARIUM", 10, 7, 2);
...
void DisplayManager::drawFooter(
  ...
  switch (page) {
    ...
    default:
      canvas_.setTextColor(AquariusTheme::SECONDARY);
      footer = outdoor.valid ? String("WIND ") + String(outdoor.windSpeedKmh, 0) + " km/h"
                           : "WEATHER WAITING";
      break;
  }
...
void DisplayManager::setDemoScene(uint8_t scene, unsigned long nowMs) { aquarium_.setDemoScene(scene, nowMs); }
void DisplayManager::clearDemo(unsigned long nowMs) { aquarium_.clearDemo(nowMs); }
const char* DisplayManager::demoSceneLabel() const { return aquarium_.demoSceneLabel(); }
