/*
 * Aquarius v0.1.1 — Touch Feeding & Cephalopods
 *
 * Focus:
...
void pollTouch(unsigned long nowMs) {
  ...
  if (touched && !touchWasDown &&
      nowMs - lastTouchMs >= AQUARIUS_TOUCH_DEBOUNCE_MS) {
    TS_Point point = touchscreen.getPoint();
    if (point.z >= AQUARIUS_TOUCH_MIN_PRESSURE) {
      int screenX = map(
          point.x,
          AQUARIUS_TOUCH_MIN_X,
          AQUARIUS_TOUCH_MAX_X,
          0,
          AquariusTheme::SCREEN_WIDTH - 1);
      int screenY = map(
          point.y,
          AQUARIUS_TOUCH_MIN_Y,
          AQUARIUS_TOUCH_MAX_Y,
          0,
          AquariusTheme::SCREEN_HEIGHT - 1);
      screenX = constrain(screenX, 0, AquariusTheme::SCREEN_WIDTH - 1);
      screenY = constrain(screenY, 0, AquariusTheme::SCREEN_HEIGHT - 1);

      lastTouchMs = nowMs;
      displayManager.feedFish(nowMs, screenX, screenY);
      Serial.printf(
          "Aquarium          : feeding triggered at %d,%d (raw %d,%d,%d)\n",
          screenX, screenY, point.x, point.y, point.z);
    }
  }
...
void setup() {
  ...
  Serial.println();
  Serial.println("========================================");
  Serial.println(" AQUARIUS v0.1.1");
