...
uint16_t blendBoot565(uint16_t a, uint16_t b, uint8_t amount) {
...
void DisplayManager::drawBootScreen(const String& status, unsigned long nowMs) {
  ...
  canvas_.drawString("v0.1.14", AquariusTheme::CENTRE_X, 87, 2);
  ...
  // A few calm boot bubbles using the same colour recipe as the main tank.
  // Keep these sparse so the boot screen remains clean and readable.
  const uint16_t bootBubble = blendBoot565(background_, TFT_CYAN, 145);
  const uint16_t bootMicroBubble = blendBoot565(background_, TFT_WHITE, 105);
  for (int i = 0; i < 4; ++i) {
    const unsigned long period = 2350UL + i * 310UL;
    const int bx = 54 + ((i * 71) % 218);
    const int by = 181 - static_cast<int>(((nowMs + i * 503UL) % period) * 67UL / period);
    const int br = (i == 1) ? 2 : 1;
    canvas_.drawCircle(bx, by, br, bootBubble);
    if (i == 2) canvas_.drawPixel(bx + 4, by + 6, bootMicroBubble);
  }
