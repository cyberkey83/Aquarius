...
void AquariumEngine::begin(unsigned long nowMs) {
  ...
  nextCalmWaveMs_ = nowMs + 240000UL + (nextRandom() % 120001UL);
  calmWaveStartedMs_ = 0;
  initialised_ = true;
}
...
const char* AquariumEngine::demoSceneLabel() const {
  static const char* labels[] = {
    ...
    "APRIL FOOL", "LEAP FISH", "NEW YEAR", "DAWN / DUSK RAY", "CALM SURFACE WAVE"
  };
...
void AquariumEngine::setDemoScene(uint8_t scene, unsigned long nowMs) {
  ...
  if (scene == 33) {
    calmWaveStartedMs_ = nowMs;
    calmWaveDirection_ = 1;
    nextCalmWaveMs_ = nowMs + 360000UL;
    return;
  }
...
void AquariumEngine::updateEvent(
  ...
  const bool safelyOffscreen = eventDirection_ > 0 ? eventX_ > TANK_RIGHT + 55 : eventX_ < TANK_LEFT - 55;
  const bool safetyTimeout = nowMs - eventStartedMs_ > 180000UL;
  if (safelyOffscreen || safetyTimeout) { eventType_ = EventType::None; scheduleNextEvent(nowMs); return; }
...
void AquariumEngine::draw(
  ...
  drawParallax(nowMs, background);
  drawBubbles(nowMs, background, clock, outdoor);
  drawSeabed(nowMs, background);
...
void AquariumEngine::drawWaterSurface(
  ...
  const bool boatActive =
      boatStartedMs_ != 0 && (nowMs - boatStartedMs_) <= 90000UL;
  const bool calmWeather =
      (!outdoor.valid || outdoor.weatherCode <= 3) &&
      ecologyRain_ < 0.08f && ecologySnow_ < 0.08f &&
      ecologyStorm_ < 0.08f && ecologyWind_ < 0.38f;

  // Trigger one gentle travelling swell roughly every 4-6 minutes.
  // It is cancelled/deferred whenever the hourly fishing boat is on screen.
  if (boatActive && calmWaveStartedMs_ != 0) calmWaveStartedMs_ = 0;
  if (calmWaveStartedMs_ == 0 && nowMs >= nextCalmWaveMs_) {
    if (calmWeather && !boatActive) {
      calmWaveStartedMs_ = nowMs;
      calmWaveDirection_ = (nextRandom() & 1U) ? 1 : -1;
    } else {
      nextCalmWaveMs_ = nowMs + 30000UL;
    }
  }

  if (calmWaveStartedMs_ != 0) {
    constexpr unsigned long WAVE_DURATION_MS = 12000UL;
    const unsigned long waveAge = nowMs - calmWaveStartedMs_;
    if (waveAge >= WAVE_DURATION_MS) {
      calmWaveStartedMs_ = 0;
      nextCalmWaveMs_ = nowMs + 240000UL + (nextRandom() % 120001UL);
    } else {
      const float p = static_cast<float>(waveAge) /
                      static_cast<float>(WAVE_DURATION_MS);
      const int crestX = calmWaveDirection_ > 0
          ? -34 + static_cast<int>(p * 388.0f)
          : 354 - static_cast<int>(p * 388.0f);
      const uint16_t crest = blend565(background, TFT_WHITE, 115);
      for (int dx = -28; dx <= 28; dx += 4) {
        const float shape = 1.0f - fabsf(dx) / 30.0f;
        const int rise = static_cast<int>(shape * shape * 4.0f);
        const int x = crestX + dx;
        if (x >= 0 && x < 320) {
          canvas_.drawPixel(x, 83 - rise, crest);
          if ((dx & 7) == 0) canvas_.drawPixel(x, 84 - rise, water);
        }
      }
    }
  }
...
void AquariumEngine::drawParallax(unsigned long nowMs, uint16_t background) {
  const uint16_t far = blend565(background, TFT_CYAN, 35);
  const uint16_t mid = blend565(background, TFT_BLUE, 55);
  for (int i=0;i<8;++i) {
    int x=(i*47 + static_cast<int>(nowMs/240UL))%340-10;
    int y=104+(i*19)%78; canvas_.drawPixel(x,y,far);
  }
  for (int i=0;i<5;++i) {
    int x=(i*71 - static_cast<int>(nowMs/150UL))%350; if(x<0)x+=350;
    int y=118+(i*23)%62; canvas_.drawFastHLine(x,y,2,mid);
  }
}

void AquariumEngine::drawSeabed(unsigned long nowMs, uint16_t background) {
  const uint16_t sand = rgb565(129, 105, 62);
  const uint16_t sandLight = rgb565(163, 137, 82);
  canvas_.fillRect(TANK_LEFT, TANK_BOTTOM - 6, TANK_RIGHT-TANK_LEFT+1, 8, sand);
  for(int i=0;i<22;++i) { int x=9+(i*29)%300; int y=TANK_BOTTOM-5+((i*7)%5); canvas_.drawPixel(x,y,(i&1)?sandLight:blend565(sand,background,60)); }
}

void AquariumEngine::drawPlants(unsigned long nowMs, uint16_t background) {
  if (plantLevel_ == 0) return;
  const float maxScale = 0.45f + static_cast<float>(plantLength_) / 15.0f * 1.95f;
  const float windRad = ecologyWindDirectionDeg_ * 0.0174532925f;
  for (uint8_t i=0;i<plantLevel_;++i) {
    const int baseX=TANK_LEFT+12+((i*67+i*i*13)%(TANK_RIGHT-TANK_LEFT-24));
    const float variety=0.62f+((i*37)%39)/100.0f;
    int height=constrain(static_cast<int>((24+i%5*5)*maxScale*variety),12,88);
    int sway=static_cast<int>(sinf(nowMs/(520.0f+i*31.0f)+i)*3.0f + cosf(windRad)*ecologyWind_*5.0f);
    uint16_t c=(i%4==1)?blend565(background,TFT_GREEN,120):(i%4==2?blend565(background,TFT_CYAN,105):blend565(background,TFT_GREEN,175));
    switch(i%4){
      case 0: canvas_.drawLine(baseX,TANK_BOTTOM-6,baseX+sway,TANK_BOTTOM-6-height,c); canvas_.drawLine(baseX+2,TANK_BOTTOM-6,baseX+5+sway,TANK_BOTTOM-12-height,c); break;
      case 1: for(int k=0;k<3;++k) canvas_.drawLine(baseX+k*3,TANK_BOTTOM-6,baseX-4+k*5+sway,TANK_BOTTOM-6-height+k*7,c); break;
      case 2: canvas_.drawLine(baseX,TANK_BOTTOM-6,baseX+sway,TANK_BOTTOM-6-height,c); for(int y=12;y<height;y+=10){canvas_.drawLine(baseX+sway*y/height,TANK_BOTTOM-6-y,baseX-6+sway*y/height,TANK_BOTTOM-10-y,c); canvas_.drawLine(baseX+sway*y/height,TANK_BOTTOM-6-y,baseX+6+sway*y/height,TANK_BOTTOM-12-y,c);} break;
      default: for(int k=-2;k<=2;++k) canvas_.drawLine(baseX,TANK_BOTTOM-6,baseX+k*3+sway,TANK_BOTTOM-18-height/2-abs(k)*3,c); break;
    }
  }
}
...
void AquariumEngine::drawCrab(unsigned long nowMs, uint16_t background) {
  ...
  const float exploreX = 28.0f + static_cast<float>((cycleNumber * 73UL) % 242UL);
  float crabX = 300.0f;
  bool visible = true;
  bool moving = true;
  bool waving = false;
...
void AquariumEngine::drawHourlyBoat(const ClockReading& clock, unsigned long nowMs, uint16_t background) {
  ...
  const int hour=clock.timeText.substring(0,2).toInt(); const int minute=clock.timeText.substring(3,5).toInt();
  if(minute==0 && hour!=lastBoatHour_){lastBoatHour_=hour;boatStartedMs_=nowMs;boatDirection_=(hour&1)?-1:1;}
  if(boatStartedMs_==0 || nowMs-boatStartedMs_>90000UL)return;
  const unsigned long t=nowMs-boatStartedMs_; const int start=boatDirection_>0?-22:342; const int stop=boatDirection_>0?72:248; const int finish=boatDirection_>0?342:-22;
  int x; if(t<28000UL)x=start+(stop-start)*(float(t)/28000.0f); else if(t<56000UL)x=stop; else x=stop+(finish-stop)*(float(t-56000UL)/34000.0f);
  const int y=80; const int d=boatDirection_>0?1:-1; const uint16_t hull=blend565(background,TFT_ORANGE,190);
  canvas_.fillTriangle(x-9,y,x+10,y,x+6,y+5,hull); canvas_.fillRect(x-3,y-5,8,5,blend565(background,TFT_WHITE,180)); canvas_.drawFastVLine(x,y-12,7,TFT_DARKGREY);
  canvas_.fillCircle(x+d*2,y-9,1,TFT_WHITE); canvas_.drawLine(x+d*2,y-8,x+d*2,y-4,TFT_DARKGREY);
  if(t>=28000UL&&t<56000UL){int twitch=((nowMs/800UL)&1)?1:0; canvas_.drawLine(x+d*2,y-7,x+d*(7+twitch),y-12,TFT_DARKGREY); canvas_.drawLine(x+d*(7+twitch),y-12,x+d*15,y-1,TFT_DARKGREY); canvas_.drawPixel(x+d*15,y+1,TFT_RED);}
  canvas_.drawFastHLine(x-d*14,y+4,8,blend565(background,TFT_CYAN,135));
}
