# AtmoQuarium v0.1.10 — Night Life & Diagnostics

Arduino IDE build for the original ILI9341 ESP32-2432S028R CYD used by the AtmoQuarium project.
...
Saved credentials from the Wi-Fi menu take priority. If no saved SSID exists AtmoQuarium falls
...
interval and weather-data age. "Render load" is the percentage of AtmoQuarium's 100 ms display
frame budget spent drawing/pushing the most recent dashboard frame; it is not a whole-chip CPU
utilisation percentage.

## Arduino IDE

Open `AtmoQuarium_v0.1.10.ino` in Arduino IDE and use the same board/library/TFT_eSPI setup that
worked for v0.1.8.
