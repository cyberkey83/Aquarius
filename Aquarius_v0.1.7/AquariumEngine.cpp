...
void AquariumEngine::configure(uint8_t fishCount, uint8_t bubbleLevel, uint8_t plantLevel,
                               ...
                               bool weatherEffects, uint8_t eventFrequency) {
  ...
  plantLength_ = constrain((int)plantLength, 1, 10);
...
void AquariumEngine::updateFish(
  ...
  for (uint8_t i = 0; i < fishCount_; ++i) {
    ...
    // v0.1.7 schooling: nearby residents gently avoid overlapping while
...
void AquariumEngine::drawBubbles(
  ...
  // v0.1.7: bubbles rise in loosely grouped streams with a gentle sine drift,
  ...
  for (uint8_t i = 0; i < bubbleCount; ++i) {
    ...
    const int drift = static_cast<int>(wave * (2.0f + ecologySurface_ * 2.0f));
...
void AquariumEngine::drawPlants(unsigned long nowMs, uint16_t background) {
  ...
  for (uint8_t i = 0; i < plantLevel_; ++i) {
    ...
    // User plant-length setting 1..10 scales the existing natural variation
    // without changing plant density. Level 5 is close to the v0.1.6 look.
    const float lengthScale = 0.45f + static_cast<float>(plantLength_) * 0.12f;
    const int height = constrain(static_cast<int>(naturalHeight * lengthScale), 6, 39);
