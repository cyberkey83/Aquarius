...
void TimeService::connectWifi() {
  if (WiFi.status() == WL_CONNECTED) return;
  lastWifiAttemptMs_ = millis();
  WiFi.mode(WIFI_STA);
  WiFi.begin(AQUARIUS_WIFI_SSID, AQUARIUS_WIFI_PASSWORD);
  const unsigned long start = millis();
  while (WiFi.status() != WL_CONNECTED && millis() - start < AQUARIUS_WIFI_CONNECT_TIMEOUT_MS) delay(250);
}
...
void TimeService::update() {
  if (WiFi.status() != WL_CONNECTED && millis() - lastWifiAttemptMs_ >= AQUARIUS_WIFI_RETRY_INTERVAL_MS) connectWifi();
  updateClock();
}
